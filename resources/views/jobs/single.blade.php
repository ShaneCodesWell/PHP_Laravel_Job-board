          ...
          <div class="row mb-5">
            <div class="col-6">
              @if(isset (Auth::user()->id))
                <form action="{{ route('save.job')}}" method="POST">
                  ...
                  @if ($savedJob > 0)
                    <button class="btn btn-block btn-success btn-md" disabled>You have saved this job!</button>
            ...
            <div class="col-6">
              <form action="{{ route('apply.job')}}" method="POST">
                ...
                @if(isset(Auth::user()->id))
                  @if ($appliedJob > 0)
                  <button class="btn btn-block btn-primary btn-md" disabled>You have already applied for this job</button>
                  @else
                  <button type="submit" name="submit" class="btn btn-block btn-primary btn-md">Apply Now</button>
                  @endif
                @else
                <a href="{{ route('login') }}" class="btn btn-block btn-primary btn-md">Log In to apply for this job</a>
                @endif
              </form>
            </div>
